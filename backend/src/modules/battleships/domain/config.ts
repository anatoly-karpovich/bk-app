...
const DEFAULT_BOARD_RULES: BattleshipsBoardRules = {
  ...
  rewards: {
    ...
    destroyBonusByShipSize: {
      1: all({ resourceId: "default", amount: 1 }),
      2: all({ resourceId: "default", amount: 1 }),
      3: all({ resourceId: "default", amount: 2 }),
      4: all({ resourceId: "default", amount: 2 }),
    },
  },
};
...
export const DEFAULT_BATTLESHIPS_RULES: BattleshipsRules = {
  ...
  boards: { [String(DEFAULT_BOARD_RULES.boardSize)]: clone(DEFAULT_BOARD_RULES) },
};
...
export function normalizeBattleshipsRules(input: BattleshipsRulesInput = {}): BattleshipsRules {
  ...
  const normalizedBoards = Object.fromEntries(
    Object.entries(sourceBoards).map(([key, board]) => {
...
export function toBattleshipsCoordinateLabel(row: number, column: number, boardSize?: number): string {
  return `${getBattleshipsBoardLetters(boardSize ?? row)[row - 1] ?? String(row)}${column}`;
}
export function formatBattleshipsShotResult(result: "miss" | "hit" | "kill"): string {
  return result === "hit" ? "Ранен" : result === "kill" ? "Убит" : "Мимо";
}
...
export function validateBattleshipsRules(rules: BattleshipsRules, resources: readonly Resource[]): void {
  ...
  Object.values(rules.boards).forEach((board) => {
    const validatePositivePool = (pool: RewardPool, context: string) => {
      ...
      if (rewards.some((reward) => reward.amount < 0))
        throw new Error(`${context}: Battleships rewards must be positive`);
    };
    validatePositivePool(board.rewards.hit, `Board ${board.boardSize} hit reward`);
    Object.entries(board.rewards.destroyBonusByShipSize).forEach(([size, pool]) =>
      validatePositivePool(pool, `Board ${board.boardSize} destroy bonus ${size}`),
    );
  });
}
...
function normalizeBattleshipsBoardRules(
  ...
  const sizes = new Set([
    ...Object.keys(DEFAULT_BOARD_RULES.rewards.destroyBonusByShipSize).map(Number),
  ...
  const destroyBonusByShipSize = Array.from(sizes).reduce<Record<number, RewardPool>>((result, size) => {
    ...
    result[size] = normalizePool(
      input.rewards?.destroyBonusByShipSize?.[size],
      legacy,
      DEFAULT_BOARD_RULES.rewards.destroyBonusByShipSize[size] ?? DEFAULT_BOARD_RULES.rewards.destroyBonusByShipSize[1],
    );
    return result;
  }, {});
  return {
    boardSize,
    ships,
    maxShots: nonNegativeInteger(input.maxShots ?? DEFAULT_BOARD_RULES.maxShots),
    rewards: { hit, destroyBonusByShipSize },
  };
}
function normalizePool(
  pool: RewardPool | undefined,
  legacyValues: Array<{ currencyId: string; value: number }> | undefined,
  fallback: RewardPool,
): RewardPool {
  ...
  if (Array.isArray(legacyValues))
    return {
      mode: "all",
      rewards: legacyValues
        .filter((value) => typeof value.currencyId === "string" && Number.isFinite(value.value))
        .map((value) => ({ resourceId: value.currencyId, amount: value.value })),
    };
  return clone(fallback);
}
function normalizeShips(ships: BattleshipsShipConfig[]): BattleshipsShipConfig[] {
  return ships
    .map((ship) => ({ size: positiveInteger(ship.size), amount: nonNegativeInteger(ship.amount) }))
    .sort((left, right) => right.size - left.size);
}
function positiveInteger(value: number): number {
  return Math.max(1, Math.floor(Number.isFinite(Number(value)) ? Number(value) : 1));
}
