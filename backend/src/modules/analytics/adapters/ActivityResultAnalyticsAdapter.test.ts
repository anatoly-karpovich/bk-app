...
function savedActivity(overrides: Partial<ActivityResultDocument> = {}): WithId<ActivityResultDocument> {
  return {
...
function createAdapter() {
  return new ActivityResultAnalyticsAdapter({ async findByProjectId() { return []; } } as never, () => "2026-08-27T12:00:00.000Z");
}

test("copies saved manual rewards, snapshots, title, revision, and immutable creation-date fallback into an Activity fact", () => {
  const fact = createAdapter().buildFact(savedActivity());
  ...
  assert.deepEqual(fact.participants, savedActivity().participants);
  assert.deepEqual(fact.resourceSnapshot, savedActivity().resourceSnapshot);
});

test("prefers the explicit manual conducted date", () => {
  const fact = createAdapter().buildFact(savedActivity({ conductedOn: "2024-01-31" }));
