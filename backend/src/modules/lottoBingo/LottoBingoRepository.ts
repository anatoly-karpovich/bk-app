...
export class LottoBingoRepository {
  async create(game: LottoBingoGame): Promise<WithId<LottoBingoGameDocument> | null> {
  ...
  async delete(gameId: string, projectId: string, expectedRevision: number): Promise<boolean> {
    return (await this.collection())
      .deleteOne({ _id: this.id(gameId), projectId, revision: expectedRevision })
  ...
  private async collection() {
    return getDefaultMongoDatabase().getCollection<LottoBingoGameDocument>(COLLECTION);
  }
  private id(value: string) {
    if (!ObjectId.isValid(value))
      throw new AppError("Invalid Lotto Bingo game id", { statusCode: 400, code: "lotto_bingo_invalid_game_id" });
    return new ObjectId(value);
  }
  private persist(game: LottoBingoGame): LottoBingoGameDocument {
    const {
      _id: _ignoredId,
      id: _ignoredPublicId,
      ...document
    } = game as LottoBingoGame & { _id?: ObjectId; id?: string };
    return document;
  }
}
