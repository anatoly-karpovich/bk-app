...
export const lottoBingoPlayerParamsSchema = z.object({ gameId: z.string().trim().min(1), playerId: z.string().trim().min(1) });
...
export const confirmLottoBingoWinnersSchema = revisionSchema.extend({ playerIds: z.array(z.string().trim().min(1)).min(1) });
