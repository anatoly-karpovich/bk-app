...
function question(
  input: Pick<QuizEventQuestion, "id" | "conductedOrder" | "reviewedAt" | "selectedAnswers" | "awards">,
): QuizEventQuestion {
  return {
    ...input,
...
test("includes only reviewed conducted questions and their persisted awards", () => {
  ...
  const unreviewed = question({
    ...
    conductedOrder: 2,
    reviewedAt: null,
    selectedAnswers: [{ playerName: "Bob", selectedMessageId: "message-2" }],
    awards: [],
  });
  const unused = question({ id: "unused", conductedOrder: null, reviewedAt: null, selectedAnswers: [], awards: [] });

  const summary = new QuizEventSummaryCalculator().calculate(
    [reviewed, unreviewed, unused],
  ...
  assert.deepEqual(
    summary.players.map((player) => [player.playerName, player.totalRewards]),
