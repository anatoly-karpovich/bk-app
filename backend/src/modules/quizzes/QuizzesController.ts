...
export class QuizzesController {
  ...
  get = async (req: Request, res: Response) =>
    this.respond(res, async () => {
      ...
      return this.service.get(req.authUser!, p.projectId, p.quizId);
    });
    ...
    this.respond(
      ...
      async () =>
        this.service.create(
          req.authUser!,
          parseRequest(projectIdParamsSchema, req.params, "Некорректный projectId").projectId,
          parseRequest(createQuizSchema, req.body, "Некорректная викторина"),
        ),
      201,
    );
  update = async (req: Request, res: Response) =>
    this.respond(res, async () => {
      ...
      return this.service.update(
        ...
        p.projectId,
        p.quizId,
        parseRequest(updateQuizSchema, req.body, "Некорректная викторина"),
      );
    });
  delete = async (req: Request, res: Response) =>
    this.respond(res, async () => {
      const p = parseRequest(quizParamsSchema, req.params, "Некорректные параметры");
      await this.service.delete(req.authUser!, p.projectId, p.quizId);
      return null;
    });
  private async respond(res: Response, action: () => Promise<unknown>, status = 200) {
    try {
      ...
      return res
        ...
        .json({
          success: false,
          message: "Не удалось обработать Quiz",
