...
const candidate = (text: string): QuizChatMessageCandidate => ({ from: "Alice", to: ["Dark"], text, timestamp: "21:00", sourceLineNumber: 1, transport: ChatTransport.DIRECT, canonicalKey: identity.createKey({ from: "Alice", to: ["Dark"], text, timestamp: "21:00" }) });
...
test("deduplicates against existing messages and within one incoming fragment", () => {
  ...
  const result = new ChatMessageDeduplicator(identity).deduplicate(existing, [candidate("old"), candidate("new"), candidate("new")]);
  assert.deepEqual(result.unique.map((item) => item.text), ["new"]);
