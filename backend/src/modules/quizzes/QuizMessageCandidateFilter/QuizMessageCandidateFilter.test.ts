...
test("filters only exact host recipients and excludes host and clan messages", () => {
  ...
  assert.deepEqual(result.map((message) => [message.from, message.transport]), [["Alice", ChatTransport.TO]]);
});

test("excludes clan messages even when clan transport is enabled", () => {
  const input = [
  ...
  assert.deepEqual(filter.filter(input, { hostNickname: "Dark", allowedTransports: [ChatTransport.CLAN] }).map((item) => item.from), []);
