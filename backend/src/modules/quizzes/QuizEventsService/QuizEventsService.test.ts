...
function setup(sourceSnapshot: QuizSnapshot = snapshot) {
  const ranker = new QuizAnswerRanker(); const engine = new QuizEventEngine(ranker, new QuizAwardCalculator(), new QuizEventSummaryCalculator(), new QuizSelectedAnswerPruner()); let event = engine.create(sourceSnapshot, { userId: "host", displayName: "Host", nickname: "Dark" }, "Event"); event.projectId = "project"; const documentId = new ObjectId();
  ...
  const identity = new ChatMessageIdentity(); const service = new QuizEventsService(repository as never, {} as never, {} as never, engine, new ChatParser(), new QuizMessageCandidateFilter(identity), new ChatMessageDeduplicator(identity), new QuizEventReadModelFactory(ranker));
...
test("saves a complete chat, preserves ids on equivalent saves, and saves the final result", async () => {
  ...
  assert.equal(first.event.state.questions[0].workflow.conductedOrder, 1); assert.equal(first.mutation.effectiveChange, true);
  ...
  const result = await service.saveQuestionResult(actor, "project", "event", questionId, [{ playerName: "Alice", selectedMessageId: messageId }], repeat.event.meta.revision);
  assert.ok(result.result.reviewedAt); assert.equal(result.event.state.questions[0].result.awards.length, 1);
});
...
test("requires reviewed results in conducted order and locks earlier results after a later result is saved", async () => {
  ...
  limitedSnapshot.questions = [
    ...limitedSnapshot.questions,
    { id: "source-2", questionIndex: 2, title: null, text: "Question 2", correctAnswer: null, attachmentUrl: null, notes: null },
  ];
  const { service, questionIds } = setup(limitedSnapshot);
  const firstChat = await service.saveQuestionChat(actor, "project", "event", questionIds[0]!, "21:00 [Alice] private [Dark] one", 0);
  const secondChat = await service.saveQuestionChat(actor, "project", "event", questionIds[1]!, "21:01 [Bob] private [Dark] two", firstChat.event.meta.revision);
  const secondMessageId = secondChat.event.state.questions[1].chat.playerGroups[0].messages[0].id;

  await assert.rejects(
    () => service.saveQuestionResult(actor, "project", "event", questionIds[1]!, [{ playerName: "Bob", selectedMessageId: secondMessageId }], secondChat.event.meta.revision),
    QuizQuestionResultOrderError,
  );

  const firstMessageId = secondChat.event.state.questions[0].chat.playerGroups[0].messages[0].id;
  const firstResult = await service.saveQuestionResult(actor, "project", "event", questionIds[0]!, [{ playerName: "Alice", selectedMessageId: firstMessageId }], secondChat.event.meta.revision);
  const secondResult = await service.saveQuestionResult(actor, "project", "event", questionIds[1]!, [{ playerName: "Bob", selectedMessageId: secondMessageId }], firstResult.event.meta.revision);
