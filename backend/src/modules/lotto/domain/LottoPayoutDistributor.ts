...
export class LottoPayoutDistributor {
  distribute(params: {
    ...
    resolvedRewards.forEach((reward) => {
      ...
      const scale = 10 ** (resource?.precision ?? 0);
      const units = Math.round(reward.amount * scale);
      const base = Math.trunc(units / playerIds.length);
      let remainder = units % playerIds.length;
      playerIds.forEach((playerId) => {
        const unitsForPlayer = base + (remainder > 0 ? 1 : 0);
        remainder -= 1;
        if (unitsForPlayer)
          amountsByPlayerId.get(playerId)!.push({ resourceId: reward.resourceId, amount: unitsForPlayer / scale });
      });
    });
    return playerIds.map((playerId) => ({
      playerId,
      place,
      resolvedRewards: structuredClone(resolvedRewards),
      awardedRewards: amountsByPlayerId.get(playerId)!,
      payoutStatus: mode,
    }));
  }
}
