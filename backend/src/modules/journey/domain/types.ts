...
export type JourneyCommentTemplateKind =
...
export interface JourneyCommentTemplate {
  id: string;
  text: string;
}
export type JourneyCommentTemplatesSnapshot = Record<JourneyCommentTemplateKind, JourneyCommentTemplate[]> &
  Partial<Record<JourneyLegacyCommentTemplateKind, JourneyCommentTemplate[]>>;
export type JourneyLastSelectedCommentIds = Partial<Record<JourneyStoredCommentTemplateKind, string>>;
export interface JourneyCommentState {
  snapshot: JourneyCommentTemplatesSnapshot;
  lastSelectedIds: JourneyLastSelectedCommentIds;
}
export interface JourneyCommentReference {
  kind: JourneyStoredCommentTemplateKind;
  templateId: string;
}
...
export interface JourneyV2AchievementEffect {
  name: string;
  requestedRewards: ResourceAmount[];
  resolvedRewards: ResourceAmount[];
  appliedRewards: ResourceAmount[];
  commentRefs: JourneyCommentReference[];
}
...
export interface JourneyV2State {
  ...
  forumLog: string[];
  commentState?: JourneyCommentState;
}
...
export interface JourneyHistoryEntryView {
  ...
  resolvedRewards: ResourceAmount[];
  appliedRewards: ResourceAmount[];
  balanceAfterRound: ResourceAmount[] | null;
...
export interface JourneyPlayer {
  ...
  movesHistory: Array<{ position: number; cell: JourneyMapCell | null; type: JourneyMoveType }>;
  removedAt: string | null;
  removedReason: string | null;
