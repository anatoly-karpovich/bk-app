...
export class JourneyForumStateFormatter {
    ...
    return [
      ...
      ...(player.bonuses.length
        ? [
            "   Бонусы:",
            ...[...player.bonuses]
              .sort((left, right) => Number(right.source === "jackpot") - Number(left.source === "jackpot"))
              .map(
                (bonus) =>
  ...
  private amounts(amounts: JourneyGameViewPlayer["balanceEntries"], game: JourneyGameView, emptyLabel = ""): string {
