...
async function migrateGameCollection(params: {
  ...
  const collection = await getDefaultMongoDatabase().getCollection(params.collectionName);
...
async function main() {
  ...
  const gameConfigsRepository = new GameConfigsRepository(getDefaultMongoDatabase());
  ...
  for (const legacyConfig of legacyConfigs) {
    ...
    migrationSummary.journeyGamesUpdated += await migrateGameCollection({
    ...
    migrationSummary.battleshipsGamesUpdated += await migrateGameCollection({
    ...
    migrationSummary.lottoGamesUpdated += await migrateGameCollection({
