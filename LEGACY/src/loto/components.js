function generateLotoPlayerInput() {
  const { cardNumbersAmount, min, max } = configurationService.getConfigForGame("loto");
  ...
  <div class="mb-3 d-flex justify-content-between" data-id="${id}">
    <div class="col-md-11 d-flex juxtify-content-around" name="loto-player">
      ...
      <div style="width: 70%">${generateTextInput(
        { placeholder: "Enter players numbers devided by comma", id: id, name: "card-numbers" },
        validationErrorMessages.LOTO_NUMBERS(cardNumbersAmount, min, max)
      )}</div>
    </div>
