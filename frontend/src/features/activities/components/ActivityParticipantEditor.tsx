...
function duplicatePlayerError(
  participant: ActivityParticipantDraft,
  participants: ActivityParticipantDraft[],
): string | null {
  ...
  return participants.filter(
    (candidate) => (candidate.playerRefId ?? candidate.nickname.trim().toLocaleLowerCase("ru")) === key,
  ).length > 1
...
function amountError(amount: ActivityResourceAmountDraft, amounts: ActivityResourceAmountDraft[]): string | null {
  ...
  return amounts.filter((candidate) => candidate.resourceId === amount.resourceId).length > 1
    ? "Ресурс повторяется."
    : null;
}
    ...
    <Stack spacing={1.25}>
      ...
      {amounts.length ? (
        amounts.map((amount, index) => {
          ...
          return (
            ...
            >
              ...
              />
              <IconButton
                aria-label={`Удалить награду ${title.toLowerCase()}`}
                color="error"
                disabled={disabled}
                onClick={() => onChange(amounts.filter((_item, itemIndex) => itemIndex !== index))}
                sx={{ mt: { sm: 0.25 }, alignSelf: { xs: "flex-end", sm: "auto" } }}
              >
        ...
        <Typography variant="body2" color="text.secondary">
          Награды этой категории пока не добавлены.
        </Typography>
      )}
    </Stack>
  );
      ...
      <CardContent>
        <Stack spacing={2}>
          {participants.length ? (
                  ...
                  <Stack spacing={2}>
                    <Stack direction="row" justifyContent="space-between" spacing={1} alignItems="flex-start">
                      ...
                      <IconButton
                        ...
                        color="error"
                        disabled={disabled}
                        onClick={() => onChange(participants.filter((_item, itemIndex) => itemIndex !== index))}
                      >
                        <DeleteOutlineRoundedIcon />
                      </IconButton>
                    </Stack>
                    ...
                    <AppResponsiveGrid columns={{ xs: 1, lg: 2 }}>
                      <RewardAmountsEditor
                        title="Обычные награды"
                        category="regular"
                        amounts={participant.rewards.regular}
                        resources={resources}
                        disabled={disabled}
                        onChange={(amounts) => updateRewards(index, "regular", amounts)}
                      />
                      <RewardAmountsEditor
                        title="Бонусные награды"
                        category="bonus"
                        amounts={participant.rewards.bonus}
                        resources={resources}
                        disabled={disabled}
                        onChange={(amounts) => updateRewards(index, "bonus", amounts)}
                      />
                    </AppResponsiveGrid>
                  </Stack>
                </Box>
            ...
            <Typography variant="body2" color="text.secondary">
              Получателей пока нет. Добавьте игрока и его итоговую награду.
            </Typography>
          )}
          <Box sx={{ alignSelf: "flex-start" }}>
            <AddPlayerButton
              onClick={() => onChange([...participants, emptyActivityParticipant(resources[0]?.id)])}
              disabled={disabled}
            />
          </Box>
        </Stack>
