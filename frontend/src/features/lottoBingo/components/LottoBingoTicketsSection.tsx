...
export default function LottoBingoTicketsSection({
  players,
  candidates,
  canDisqualify,
  canRestore,
        ...
        <Stack spacing={1.5}>
              ...
              {filters.map((item) => (
                <AppPillButton
                  ...
                  sx={{
                    minHeight: 32,
                    border: "1px solid",
                    borderColor: filter === item.value ? "primary.main" : "divider",
                    bgcolor: filter === item.value ? "#eef2ff" : "common.white",
                    fontSize: "0.72rem",
                  }}
                >
                  {item.label}
          ...
          <Stack spacing={0.25}>
            <Typography variant="h5">
              {isRegistration ? "Выданные билеты" : "Билеты игроков"}{" "}
            ...
            {isRegistration && (
              <Typography variant="caption" color="text.secondary">
                "Откройте билет крупно и сделайте аккуратный скрин для игрока."
              </Typography>
            )}
          </Stack>
          <LottoBingoTicketGrid
            ...
            onRemove={onRemove}
            onOpen={onOpen}
            emptyMessage={isRegistration ? "Выданных билетов пока нет." : undefined}
          />
        </Stack>
      </CardContent>
    </Card>
