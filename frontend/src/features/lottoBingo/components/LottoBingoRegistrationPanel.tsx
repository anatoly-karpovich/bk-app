...
export default function LottoBingoRegistrationPanel({ players, playerName, busy, canAddPlayer, canRemovePlayer, canStart, onPlayerNameChange, onAddPlayer, onRemovePlayer, onStart }: Props) {
  ...
  const isDuplicatePlayer = normalizedPlayerName.length > 0 && players.some(
    (player) => player.nickname.trim().toLocaleLowerCase() === normalizedPlayerName,
  );
      ...
      <CardContent sx={{ px: { xs: 2.25, md: 2.5 }, pt: 2, pb: { xs: 2.25, md: 2.5 } }}>
        <Stack direction={{ xs: "column", sm: "row" }} spacing={1} alignItems={{ sm: "center" }}>
          <Box sx={{ flex: 1, minWidth: 0 }}>
            <GamePlayerNameInput label="Ник игрока" value={playerName} onChange={onPlayerNameChange} disabled={busy || !canAddPlayer} errorText={isDuplicatePlayer ? "Игрок уже добавлен" : null} showErrorInLabel helperTextMode="hidden" placeholder="Ник игрока" />
          </Box>
          <AddPlayerButton variant="contained" disabled={busy || !canAddPlayer || !playerName.trim() || isDuplicatePlayer} onClick={onAddPlayer} sx={{ minWidth: { sm: 140 } }} />
        </Stack>
