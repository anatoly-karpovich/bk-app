...
import {
  getQuizQuestionStateLabel,
  getQuizQuestionStateTone,
  getShortQuizQuestionText,
} from "./quizEventWorkspace.helpers";
        ...
        <Stack spacing={1}>
          ...
          {questions.map((question) => (
            <AppSelectableListItem
              ...
              secondaryText={`${getQuizQuestionStateLabel(question)} · ${getShortQuizQuestionText(question.questionText)}`}
              icon={<CheckCircleRoundedIcon fontSize="small" />}
              selected={question.id === selectedQuestionId}
              onClick={() => onSelectQuestion(question.id)}
              trailing={
                <Chip size="small" color={getQuizQuestionStateTone(question)} label={question.ranking.length} />
              }
            />
          ))}
        </Stack>
