  ...
  return (
    <Box
      sx={{
        ...
        borderRadius: 2.25,
        p: 1.75,
        bgcolor: isSelected ? "rgba(79, 70, 229, 0.025)" : "background.paper",
      ...
      <Stack spacing={0.75}>
        <Stack direction="row" alignItems="center" justifyContent="space-between" spacing={1}>
          ...
          <Typography variant="caption" color="text.secondary" noWrap>
            {group.messages.length} {group.messages.length === 1 ? "ответ" : "ответа"}
          </Typography>
        </Stack>
        ...
        {group.messages.map((message) => (
          <Stack
            ...
            sx={{
              pt: 0.75,
              borderTop: hasAlternatives ? 1 : 0,
              borderStyle: hasAlternatives ? "dashed" : "solid",
              borderColor: hasAlternatives ? "divider" : "transparent",
            }}
          >
            {hasAlternatives ? (
              <Radio
                ...
                onChange={() => onSelectedMessage(message.id)}
              />
            ) : (
              <Box sx={{ width: 32, flexShrink: 0 }} />
            )}
            <Typography variant="caption" color="text.secondary" sx={{ width: 42, pt: 0.75, flexShrink: 0 }}>
              {message.timestamp ?? "—"}
            </Typography>
            <Typography variant="caption" color="text.secondary" sx={{ width: 42, pt: 0.75, flexShrink: 0 }}>
              {message.transport ?? ""}
            </Typography>
            <Typography variant="body2" sx={{ pt: 0.55, overflowWrap: "anywhere" }}>
              {message.text}
            </Typography>
          </Stack>
        ))}
...
export default function QuizAnswerSelectionEditor({
  question,
  draft,
  dirty,
  editable,
  requiredQuestionIndex,
  busy,
  onPlayerSelected,
  ...
  expanded,
  onExpandedChange,
}: QuizAnswerSelectionEditorProps) {
  const selectedPlayersCount = question.playerGroups.filter(
    (group) => draft?.[group.playerName]?.isSelected ?? group.selectedMessageId !== null,
  ).length;
  const allPlayersSelected = question.playerGroups.length > 0 && selectedPlayersCount === question.playerGroups.length;
  const canSaveResult =
    question.conductedOrder !== null && requiredQuestionIndex === null && (dirty || question.reviewedAt === null);
  ...
  return (
    ...
    >
      <AccordionSummary
        ...
        sx={{ px: { xs: 2, sm: 2.75 }, py: 0.5, "& .MuiAccordionSummary-content": { my: 1.5 } }}
      >
        <Stack spacing={0.5}>
          <Stack direction="row" spacing={1} alignItems="center" flexWrap="wrap" useFlexGap>
            ...
            <AppChip
              size="small"
              label={`${selectedPlayersCount} выбрано`}
              color="primary"
              sx={{ bgcolor: "rgba(79, 70, 229, 0.1)", color: "primary.main" }}
            />
          </Stack>
          <Typography variant="body2" color="text.secondary">
            Отметьте верные ответы. При нескольких сообщениях выберите одно.
          </Typography>
        </Stack>
      </AccordionSummary>
      <AccordionDetails sx={{ px: { xs: 2, sm: 2.75 }, pt: 0, pb: 2.75, borderTop: 1, borderColor: "divider" }}>
        <Stack spacing={1.75} sx={{ pt: 2 }}>
          {requiredQuestionIndex !== null ? (
            <Alert severity="warning">{quizTexts.event.reviewOrderBlocked(requiredQuestionIndex)}</Alert>
          ) : null}
          {question.playerGroups.length ? (
            <Stack direction="row" justifyContent="space-between" alignItems="center" flexWrap="wrap" useFlexGap>
              <Typography variant="body2" fontWeight={700}>
                Выбрано {selectedPlayersCount} из {question.playerGroups.length}
              </Typography>
              <AppPillButton size="small" variant="text" disabled={!editable || busy} onClick={setAllPlayersSelected}>
                {allPlayersSelected ? "Снять выбор" : "Выбрать всех"}
              </AppPillButton>
            </Stack>
          ) : null}
          {question.playerGroups.length ? (
            <Box
              sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "repeat(2, minmax(0, 1fr))" }, gap: 1.5 }}
            >
              {question.playerGroups.map((group) => (
                <PlayerAnswerGroup
                  ...
                  onSelectedMessage={(selectedMessageId) =>
                    onPlayerSelectedMessage(group.playerName, selectedMessageId)
                  }
                />
              ))}
            </Box>
            ...
            <Typography variant="body2" color="text.secondary">
              Пока нет распознанных кандидатов.
            </Typography>
          )}
          ...
          {editable ? (
            <Box
              sx={{
                ...
                zIndex: 1,
                display: "flex",
                justifyContent: dirty ? "space-between" : "flex-end",
                alignItems: "center",
                gap: 1.5,
                flexWrap: "wrap",
                mt: 0.5,
            ...
            >
              ...
              <AppPillButton variant="contained" disabled={busy || !canSaveResult} onClick={onSave}>
                Сохранить результат
              </AppPillButton>
            </Box>
          ) : null}
          {canRequestRecheck ? (
            <Stack direction="row" justifyContent="flex-end" sx={{ mt: 0.5 }}>
              <AppPillButton variant="outlined" disabled={busy} onClick={onRequestRecheck}>
                Перепроверить
              </AppPillButton>
            </Stack>
          ) : null}
