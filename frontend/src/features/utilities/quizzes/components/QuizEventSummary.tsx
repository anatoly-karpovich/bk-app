...
export default function QuizEventSummary({ event }: { event: QuizEvent }) {
  ...
  const summaryText =
    ...
    summary.players
      .map(
        (player) =>
          `${player.playerName} — ${formatResourceAmounts(player.totalRewards, event.quizSnapshot.resources) || "без награды"}`,
      )
      .join("\n");
        ...
        <Stack spacing={2.25}>
          ...
          >
            <Stack spacing={0.5}>
              ...
              <Typography variant="body2" color="text.secondary">
                Проверено {event.reviewedQuestionsCount} из {event.conductedQuestionsCount} проведённых вопросов.
              </Typography>
            </Stack>
            <AppPillButton
              size="small"
          ...
          <Stack direction={{ xs: "column", sm: "row" }} spacing={1.5}>
            {[
              ...
              <Box
                key={label as string}
                sx={{ flex: 1, p: 1.75, border: 1, borderColor: "divider", borderRadius: 2, bgcolor: "#fafbfc" }}
              >
              ...
              <TableBody>
                {summary.players.map((player) => (
                  <TableRow key={player.playerName} hover>
                    ...
                    <TableCell>
                      {formatResourceAmounts(player.regularRewards, event.quizSnapshot.resources) || "—"}
                    </TableCell>
                    <TableCell sx={{ color: "#a56f00", fontWeight: 700 }}>
                      {formatResourceAmounts(player.bonusRewards, event.quizSnapshot.resources) || "—"}
                    </TableCell>
                    <TableCell sx={{ fontWeight: 700 }}>
                      {formatResourceAmounts(player.totalRewards, event.quizSnapshot.resources) || "—"}
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
