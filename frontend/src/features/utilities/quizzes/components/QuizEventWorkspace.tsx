...
export default function QuizEventWorkspace({ event, selectedQuestionId, selectionDrafts, isSelectionDraftDirty, busy, editable, onSelectQuestion, onPlayerSelected, onPlayerSelectedMessage, onSaveResult, onSaveChat, onMarkAsNotConducted, onRequestComplete, onReopen, onRequestDelete }: QuizEventWorkspaceProps) {
  ...
  const [expandedSection, setExpandedSection] = useState<"answers" | "result" | null>("answers");
  const canMutate = editable && event.status === "open";
  useEffect(() => {
    setExpandedSection(question?.reviewedAt ? "result" : "answers");
  ...
  const nextQuestion = () => { const next = event.questions.find((item) => item.conductedOrder === null); if (next) onSelectQuestion(next.id); };
  return <><Stack spacing={2.25}>
    ...
    <Stack direction={{ xs: "column", lg: "row" }} spacing={2.25} alignItems="flex-start"><QuizEventQuestionNavigation questions={event.questions} selectedQuestionId={question.id} onSelectQuestion={onSelectQuestion} /><Stack spacing={2.25} sx={{ flex: 1, minWidth: 0 }}><Card><CardContent><Stack spacing={1.5}><Stack direction={{ xs: "column", sm: "row" }} justifyContent="space-between" alignItems={{ sm: "center" }}><Typography variant="h5">Вопрос {question.questionIndex}</Typography><Chip label={getQuizQuestionStateLabel(question)} color={getQuizQuestionStateTone(question)} /></Stack>{canMutate ? <Stack direction={{ xs: "column", sm: "row" }} spacing={1}><AppPillButton disabled={busy || event.firstUnconductedQuestionId === null} variant="outlined" onClick={nextQuestion}>Следующий вопрос</AppPillButton>{question.conductedOrder !== null ? <AppPillButton disabled={busy} variant="outlined" color="warning" onClick={() => setConfirmMarkNotConducted(true)}>Считать вопрос непроведённым</AppPillButton> : null}</Stack> : null}<Divider /><QuizMessagePreviews question={question} /></Stack></CardContent></Card>
      <QuizChatControls key={question.id} question={question} busy={busy} editable={canMutate} onSave={onSaveChat} />
      <QuizAnswerSelectionEditor question={question} draft={selectionDrafts[question.id]} dirty={isSelectionDraftDirty(question.id)} editable={canMutate && question.reviewedAt === null} busy={busy} expanded={expandedSection === "answers"} onExpandedChange={(expanded) => setExpandedSection(expanded ? "answers" : null)} onPlayerSelected={(playerName, isSelected) => onPlayerSelected(question.id, playerName, isSelected)} onPlayerSelectedMessage={(playerName, selectedMessageId) => onPlayerSelectedMessage(question.id, playerName, selectedMessageId)} onSave={() => onSaveResult(question.id)} />
      <QuizQuestionResult question={question} resources={event.quizSnapshot.resources} expanded={expandedSection === "result"} onExpandedChange={(expanded) => setExpandedSection(expanded ? "result" : null)} />
