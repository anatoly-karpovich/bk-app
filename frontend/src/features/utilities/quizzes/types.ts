...
export interface QuizEvent { id: string; quizId: string; name: string; hostSnapshot: { nickname: string }; status: QuizEventStatus; currentQuestionId: string | null; createdAt: string; updatedAt: string; questions: QuizEventQuestion[]; quizSnapshot: { resources: ProjectResource[] }; summary: { players: Array<{ playerName: string; correctAnswers: number; regularRewards: ResourceAmount[]; bonusRewards: ResourceAmount[]; totalRewards: ResourceAmount[] }>; totalRewards: ResourceAmount[]; completedQuestions: number; totalQuestions: number; totalUniqueCorrectAnswers: number } | null; }
