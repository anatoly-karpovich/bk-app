...
export default function QuizConfigDescriptionSection({
  source,
  draft,
  disabled,
  onChange,
}: QuizConfigDescriptionSectionProps) {
  ...
  const summaryItems = [
    { label: "Конфиг", value: draft.name.trim() || quizConfigsTexts.card.untitled },
    {
      ...
      value: isQuestionCountMissing
        ? quizConfigsTexts.card.notConfigured
        : quizConfigsTexts.card.questionsValue(draft.questionCount ?? 0),
    },
    {
      label: texts.status,
      value: draft.status === "ready" ? quizConfigsTexts.card.ready : quizConfigsTexts.card.draft,
    },
  ];
  ...
  return (
    <Stack spacing={2.25}>
        ...
        <CardContent>
          <Stack spacing={2}>
            ...
            <AppResponsiveGrid columns={{ xs: 1, md: 2 }}>
              ...
              <AppTextInput
                fullWidth
                label={texts.configDescription}
                value={draft.description}
                changed={draft.description !== source.description}
                disabled={disabled}
                placeholder="Коротко опишите назначение конфига"
              ...
              <AppTextInput
                ...
                error={isQuestionCountMissing}
                disabled={disabled}
                inputProps={{ min: 1, step: 1 }}
                placeholder="10"
                onChange={(event) =>
                  onChange({ questionCount: event.target.value ? Number(event.target.value) : null })
                }
              />
              <Card variant="outlined" sx={{ bgcolor: "rgba(248, 250, 252, 0.9)", boxShadow: "none" }}>
                <CardContent sx={{ py: 1.25, "&:last-child": { pb: 1.25 } }}>
                  ...
                  <Typography variant="body2" fontWeight={700}>
                    {draft.status === "ready" ? "Готов" : "Черновик — есть незаполненные обязательные поля"}
                  </Typography>
                </CardContent>
              </Card>
            </AppResponsiveGrid>
      ...
      <ConfigSummaryCard
        ...
        description="Быстрый просмотр ключевых параметров конфига."
        items={summaryItems}
      />
